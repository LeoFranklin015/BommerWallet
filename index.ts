...
import { HELP_MESSAGE, GMX_HELP_MESSAGE, ROOTSTOCK_HELP_MESSAGE } from "./utils/constants";
...
// Initialize WalletManager for multi-chain support (Rootstock and Arbitrum Sepolia)
const walletManager = new WalletManager(client);

// Initialize RootstockWalletManager for Rootstock-specific functionality
const rootstockManager = new RootstockWalletManager(process.env.ROOTSTOCK_NETWORK as "mainnet" | "testnet" || "testnet");
...
(async (): Promise<void> => {
  ...
  // Handle WalletConnect and Rootstock messages
  client.on("message", async (message) => {
    try {
      ...
      // Try to handle Rootstock commands
      const rootstockResponse = await rootstockManager.handleMessage(message);
  ...
  client.on("message_create", async (message) => {
    ...
    try {
      ...
      if (command === "rootstock help") {
        await message.reply(ROOTSTOCK_HELP_MESSAGE);
        return;
      }
      ...
      if (!await walletManager.getWalletAddress(message.from.toString())) {
        await message.reply(
          "You don't have a wallet yet! Please create one first by sending 'create wallet' or 'create rootstock wallet'."
        );
        return;
      }

      // For any other command, use a simple response
      await message.reply("I didn't understand that command. Please try 'help' for a list of available commands.");
  ...
  const initializeWithRetry = async () => {
    try {
      ...
      if (initAttempts < maxAttempts) {
        console.log(`Retrying initialization (attempt ${initAttempts + 1}/${maxAttempts})...`);

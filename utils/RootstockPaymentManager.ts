...
export class RootstockPaymentManager {
  ...
  constructor(network: "mainnet" | "testnet" = "testnet") {
    ...
    this.provider = new ethers.JsonRpcProvider(
      ROOTSTOCK_NETWORKS[network].rpcUrl
    );
  ...
  public async init(privateKey: string): Promise<boolean> {
    try {
      ...
      console.log(`Initialized Rootstock wallet: ${address}`);
      console.log(`Wallet balance: ${ethers.formatEther(balance)} RBTC`);
      
      return true;
    } catch (error) {
      console.error("Failed to initialize Rootstock payment manager:", error);
      return false;
    }
  }
  ...
  /**
   * Process a wallet-to-wallet transfer on Rootstock
   */
  public async transferFunds(
    ...
    try {
      ...
      throw new Error(`Failed to transfer funds: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  public async waitForConfirmation(
    ...
    try {
      ...
      if (receipt && receipt.status === 1) {
        ...
        console.log(`Payment ${paymentId} confirmed with ${requiredConfirmations} confirmations`);
  ...
  public async checkPaymentReceived(
    ...
    try {
      ...
      console.log(`Checking for payment of ${expectedAmount} RBTC to ${address}...`);
      ...
      for (let i = blockNumber; i >= startBlock; i--) {
        ...
        for (const transaction of block.transactions) {
          ...
          if (tx.to?.toLowerCase() === address.toLowerCase() && tx.value >= amountWei) {
  ...
  public getExplorerUrl(txHash: string): string {
    return `${ROOTSTOCK_NETWORKS[this.network].blockExplorer}/tx/${txHash}`;
  }
} 
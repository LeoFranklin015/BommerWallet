...
// For compatibility with dApps that don't natively support Rootstock
const CHAIN_MAPPING: Record<number, number> = {
  // Map common chains to Rootstock Testnet
  1: 31337,     // Ethereum -> Rootstock Testnet
  42161: 31337, // Arbitrum -> Rootstock Testnet
  137: 31337,   // Polygon -> Rootstock Testnet
  10: 31337,    // Optimism -> Rootstock Testnet
  56: 31337,    // BSC -> Rootstock Testnet
  43114: 31337, // Avalanche -> Rootstock Testnet
};
  ...
  private async handleSessionProposal(
    ...
    try {
      ...
      const handleNamespace = (namespace: EVMNamespace, chains: string[]) => {
        // Map requested chains to supported Rootstock chain
        const mappedChains = chains.map(chain => {
          ...
          if (requestedChainId in SUPPORTED_CHAINS) {
            ...
            console.log(`Mapping chain ${requestedChainId} to Rootstock ${mappedChainId}`);
            return `eip155:${mappedChainId}`;
          }
          return null;
        }).filter(Boolean) as string[];

        // If no chains are supported and none can be mapped, default to Rootstock testnet
        if (mappedChains.length === 0) {
          console.log(`No supported chains found, defaulting to Rootstock testnet`);
          mappedChains.push(`eip155:31337`);
        }
      ...
      const supportedChainsMsg = `• ${rootstockTestnet.name} (31337)`;
    ...
    try {
      ...
      if (emoji === "👍") {
        if (pendingRequest.type === "transaction") {
          ...
          const requestedChainId = parseInt(request.params.chainId.split(":")[1]);
          
          // Map to Rootstock testnet if needed
          const chainId = requestedChainId in SUPPORTED_CHAINS 
            ? requestedChainId 
            : 31337; // Always use Rootstock testnet
